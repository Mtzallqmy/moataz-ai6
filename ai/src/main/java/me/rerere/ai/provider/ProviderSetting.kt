...
@Serializable
enum class OpenAICompatiblePreset {
    OPENAI,
    NVIDIA,
    HUGGING_FACE,
    CUSTOM,
...
@Serializable
data class OpenAICompatibilityCapabilities(
    ...
    val embeddings: Boolean = true,
    val responsesApi: Boolean = true,
...
sealed class ProviderSetting {
    ...
    data class OpenAI(
        ...
        // Legacy credential field kept for backward compatibility with existing settings,
        // imports and older backups. New UI writes the primary key here as well, while the
        // structured pool below carries labels, enablement and selection policy.
        var apiKey: String = "",
        var apiKeys: List<ApiKeyEntry> = emptyList(),
        var apiKeyStrategy: ApiKeyStrategy = ApiKeyStrategy.ROUND_ROBIN,
        var primaryApiKeyId: String? = null,
        var manualApiKeyId: String? = null,
        var failoverOnRateLimit: Boolean = false,
        // Backward-compatible default: older OpenAI-compatible configs were allowed to
        // send requests with a blank credential. Official presets override this to false.
        var allowEmptyApiKey: Boolean = true,
        var baseUrl: String = "https://api.openai.com/v1",
        var chatCompletionsPath: String = "/chat/completions",
        var compatibilityPreset: OpenAICompatiblePreset = OpenAICompatiblePreset.CUSTOM,
        var customHeaders: List<CustomHeader> = emptyList(),
        var organizationId: String = "",
        var projectId: String = "",
        var compatibilityCapabilities: OpenAICompatibilityCapabilities = OpenAICompatibilityCapabilities(),
