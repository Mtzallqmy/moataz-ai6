...
class SkillUrlImporter(
    ...
    private fun fetch(url: String): String {
        val req = Request.Builder()
            .url(url)
            .header("User-Agent", "moataz-ai/skill-importer")
    ...
    private fun transcodeFromHermes(raw: String, sourceUrl: String, override: String?): String? {
        ...
        val body = buildString {
            ...
            appendLine("## Source")
            appendLine()
            appendLine("Imported from Hermes skill at $sourceUrl. Tool names were not transcoded — Hermes references its own tool surface, which doesn't always map 1:1 to Moataz AI's. Edit any tool references manually if the skill misbehaves.")
        }
