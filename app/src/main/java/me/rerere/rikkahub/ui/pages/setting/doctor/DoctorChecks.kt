...
class DoctorChecks(
        ...
        if (termuxInstalled) {
            ...
            add(
                DoctorCheck(
                    ...
                    detail = if (runCommandPerm) "Granted — Moataz AI can dispatch shell commands to Termux."
    ...
    private fun diagnosticsChecks(enabled: Set<LocalToolOption>): List<DoctorCheck> = listOf(
        DoctorCheck(
            ...
            label = "App build",
            detail = "Moataz AI ${BuildConfig.VERSION_NAME} (${BuildConfig.VERSION_CODE}) — debug=${BuildConfig.DEBUG}",
            severity = Severity.INFO,
        ),
        DoctorCheck(
            id = "diag.android",
