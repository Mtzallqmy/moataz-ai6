...
private const val BACKUP_PREFIX = "moataz_ai_backups/"
private const val LEGACY_BACKUP_PREFIX = "rikkahub_backups/"
...
class S3Sync(
    ...
    suspend fun backupToS3(config: S3Config) = withContext(Dispatchers.IO) {
        ...
        val client = getS3Client(config)
        val key = "$BACKUP_PREFIX${file.name}"
    ...
    suspend fun listBackupFiles(config: S3Config): List<S3BackupItem> = withContext(Dispatchers.IO) {
        val client = getS3Client(config)
        val current = client.listObjects(
            prefix = BACKUP_PREFIX,
            maxKeys = 1000
        ).getOrThrow()
        val legacy = runCatching {
            client.listObjects(prefix = LEGACY_BACKUP_PREFIX, maxKeys = 1000).getOrThrow()
        }.getOrNull()

        (current.objects + legacy?.objects.orEmpty())
            .filter { obj ->
                (obj.key.startsWith("${BACKUP_PREFIX}backup_") || obj.key.startsWith("${LEGACY_BACKUP_PREFIX}backup_")) &&
