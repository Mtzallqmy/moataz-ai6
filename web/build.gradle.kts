...
// Install web-ui dependencies from the tracked pnpm lockfile. GitHub Actions installs
// both Bun and pnpm, but dependency resolution stays on pnpm so a clean build never needs
// to generate/migrate a second lockfile.
val installWebUiDeps = tasks.register<Exec>("installWebUiDeps") {
    ...
    when {
        Os.isFamily(Os.FAMILY_WINDOWS) -> commandLine("cmd", "/c", "pnpm", "install", "--frozen-lockfile")
        else -> commandLine("pnpm", "install", "--frozen-lockfile")
    }
